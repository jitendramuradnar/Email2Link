...
app.post(properties.api + '/login', function(req, res, md5pwd) {
    const db_Email2Link_db = require('../db/Email2Link_db_schema.js');
...
app.post(properties.api + '/changePassword', function(req, res) {
    ...
    db_Email2Link_db.User.findOne({ username: req.user.username, password: req.body.passwordOld }).exec(function(err, user) {
...
var getUser = function(username, password, callback) {
    ...
    db_Email2Link_db.User.findOne({ username: username, password: password }).lean().exec(function(err, user) {
        if (user)
            user.password = undefined;
        callback(user);
    });
...
var createUser = function() {
    ...
    db_Email2Link_db = require('../db/Email2Link_db_schema.js');
    db_Email2Link_db.User.count().exec(function(err, count) {
        if (count == 0) {
            var admin = new db_Email2Link_db.User({
                username: 'admin',
